...
func main() {
	...
	resp, err := c.MicrosoftUpdatesAPI.StandaloneBeta.GetLatestV1(context.Background())
	if err != nil {
		log.Fatalf("StandaloneBeta.GetLatestV1: %v", err)
	}

	fmt.Printf("Found %d beta packages\n\n", len(resp.Packages))
