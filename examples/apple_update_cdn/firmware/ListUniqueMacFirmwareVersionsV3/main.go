...
func main() {
	fmt.Println("=== Apple Update CDN - List Unique Mac Firmware Versions V3 ===")
	...
	// Returns one entry per unique macOS build ID, sorted newest-first.
	// All Apple Silicon Macs share a single Universal IPSW per version.
	versions, _, err := c.AppleUpdateCDNAPI.Firmware.ListUniqueMacFirmwareVersionsV3(ctx)
	if err != nil {
		log.Fatalf("Error listing unique Mac firmware versions: %v", err)
	}

	fmt.Printf("Found %d unique macOS firmware versions\n", len(versions))
