...
import (
	...
	"github.com/deploymenttheory/go-api-sdk-apple/microsoft_updates/microsoft_updates_api/appstore_macos/mocks"
	"github.com/jarcoal/httpmock"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func setupMockClient(t *testing.T) *appstore_macos.AppStoreMacOSService {
	t.Helper()
	...
	return appstore_macos.NewService(transport)
}
...
func TestGetAppByNameV1_Success(t *testing.T) {
	...
	assert.Equal(t, "com.microsoft.Word", app.BundleID)
	assert.NotEmpty(t, app.Version)
	assert.NotEmpty(t, app.TrackViewURL)
}

func TestGetAppByNameV1_NotFound(t *testing.T) {
	svc := setupMockClient(t)
	mocks.RegisterMicrosoftWordMock()

	ctx := context.Background()
	_, err := svc.GetAppByNameV1(ctx, "Nonexistent App")
	require.Error(t, err)
...
func TestBundleIDConstants(t *testing.T) {
	assert.Equal(t, "com.microsoft.Word", appstore_macos.BundleIDWord)
	assert.Equal(t, "com.microsoft.Excel", appstore_macos.BundleIDExcel)
	assert.Equal(t, "com.microsoft.edgemac", appstore_macos.BundleIDEdge)
}
